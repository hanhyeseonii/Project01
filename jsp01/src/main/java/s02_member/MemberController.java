...
public class MemberController extends HttpServlet {
	...
	MemberDAO mdao = new MemberDAO(); //dao객체 필드로 생성
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (uri.contains("insert")) {
			...
			//forward방식(대량의 데이터 전송)
			request.setAttribute("mlist",mlist);
			request.getRequestDispatcher("/view/db/20220330_02_selectList.jsp").forward(request, response);
			...
			request.getRequestDispatcher("/view/db/20220330_03_update.jsp").forward(request, response);
