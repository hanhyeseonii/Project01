...
public class CultureService	{
	...
	Map<String, Double> map = new HashMap<String, Double>();
		...
		try {
	        ...
	        conn.setRequestMethod("GET");
	        conn.setRequestProperty("Content-type", "application/json"); 
	        conn.setRequestProperty("X-NCP-APIGW-API-KEY-ID", "3cqp3a70jr"); //앱 등록 시 발급받은 Client ID
	        conn.setRequestProperty("X-NCP-APIGW-API-KEY", "ZrUdrsONeqJozNLIV6cvA5rzsJFe1g9rKUq1hOky"); //앱 등록 시 발급 받은 Client Secret
            ...
            long totalCount= (long)object.get("totalCount");
            System.out.println(totalCount);
            if(totalCount==0) return null; //데이터가 존재하지 않을 시 메소드 종료
